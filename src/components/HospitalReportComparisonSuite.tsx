...
Directive: Immediate spatial segregation required. Negative-pressure isolation for Bed 04.`;
  ...
  const filteredRows = comparativeRows.filter((row) => {
    if (astFilter === "shared-resistant") return row.crossCompatibility === "Shared Resistance";
    if (astFilter === "sensitive") return row.crossCompatibility === "Sensitive to Both";
    return true;
  });

  return (
    <section id="hospital-report-suite" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8 py-6">
      {/* 1. Header Banner with Hospital Accreditation & Dual Mode Switcher */}
      ...
      {/* 2. PATIENT / FAMILY VIEW (If active: Accessible, Reassuring, Clear Language) */}
      {viewMode === "patient" && (
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h2 className="text-base sm:text-lg font-bold text-slate-900 dark:text-white">
                  Patient &amp; Family Medical Report Guide (आसान भाषा में रिपोर्ट का विवरण)
                </h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-white dark:bg-slate-900/90 border border-teal-200 dark:border-teal-500/30 rounded-xl p-5 space-y-3 shadow-sm">
              ...
              <div className="text-xs text-slate-700 dark:text-slate-200 space-y-2.5 leading-relaxed">
                <p>
                  🔬 <strong>Bacteria Found:</strong> {patient1.pathogen}. Yeh bacteria aam dawaiyon jaise standard penicillin aur cephalosporin se khatam nahi ho raha hai.
                </p>
                <p>
                  💊 <strong>Kyu Regular Dawai Band Ki Gayi:</strong> Report me dekha gaya hai ki bacteria ne aam antibiotic ke khilaf immunity (resistance) bana li hai. Doctors ne aapke kidney function (eGFR: {patient1.clinicalParams?.bloodReport.eGfr || 42} mL/min) aur blood parameters check karke safe targeted antibiotic select kiya hai.
                </p>
                <p className="text-teal-800 dark:text-teal-300 font-medium">
                  ✅ <strong>Safe Dawa:</strong> Ceftazidime-Avibactam / Polymyxin regimen start kiya gaya hai jo is strain par 100% effective paya gaya hai.
                </p>
              </div>
            </div>
            ...
            <div className="bg-white dark:bg-slate-900/90 border border-cyan-200 dark:border-cyan-500/30 rounded-xl p-5 space-y-3 shadow-sm">
              ...
              <div className="text-xs text-slate-700 dark:text-slate-200 space-y-2.5 leading-relaxed">
                <p>
                  🔬 <strong>Bacteria Found:</strong> {patient2.pathogen}. Yeh ICU hospital strain hai jo multiple medicines ke khilaf resistant hai.
                </p>
                <p>
                  🛑 <strong>Kyu Dono Mareezon Ko Alag Rakha Gaya Hai:</strong> Kyunki dono mareezon ke bacteria mein milte-julte resistance gene hain. Agar dono ko ek hi ward me paas rakha jaye toh bacteria ek doosre me transfer ho sakta hai. Isliye hospital infection control team ne safe spatial distance banayi hai.
                </p>
                <p className="text-cyan-800 dark:text-cyan-300 font-medium">
                  🛡️ <strong>Attendants ke liye Hidayat:</strong> Mareez se milte waqt hamesha gloves aur mask pehnein, aur room se bahar aane par hath ache se sanitize karein.
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          >
            ...
            <div className="bg-slate-50 dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800/80 rounded-xl p-3 grid grid-cols-2 gap-2 text-[11px]">
              ...
              <div>
                <span className="text-slate-500 dark:text-slate-400 block text-[10px]">SPECIMEN / SITE:</span>
                <span className="text-cyan-700 dark:text-cyan-300 font-medium">{patient1.specimen}</span>
              </div>
              <div>
                <span className="text-slate-500 dark:text-slate-400 block text-[10px]">COLLECTION DATE:</span>
                <span className="text-slate-700 dark:text-slate-300">{patient1.collectionDate}</span>
              </div>
            </div>

            {/* Organism & Microscopy */}
            <div className="bg-slate-50/70 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-xl p-3 space-y-1.5">
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-mono uppercase text-slate-500 dark:text-slate-400 font-bold">ISOLATED ORGANISM:</span>
                <span className="px-2 py-0.5 rounded bg-rose-100 dark:bg-rose-500/20 text-rose-800 dark:text-rose-300 border border-rose-300 dark:border-rose-500/30 font-bold text-[11px]">
                  Gram-Negative Bacilli
                </span>
              </div>
              <div className="text-sm font-bold text-slate-900 dark:text-white font-mono">{patient1.pathogen}</div>
              <div className="text-[11px] text-slate-500 dark:text-slate-400 flex items-center justify-between">
                <span>Microscopy: &gt;25 WBCs/HPF, Gram-negative rods</span>
                <span>Colony Count: &gt;10⁵ CFU/mL</span>
              </div>
            </div>

            {/* Antibiogram (AST) Table */}
                ...
                <table className="w-full text-left text-[11px]">
                  ...
                  <tbody className="divide-y divide-slate-200 dark:divide-slate-800/60">
                    {patient1.antibiotics.map((abx, i) => {
                      const isR = abx.status === "Resistant";
                      const isS = abx.status === "Sensitive";
                      return (
            ...
            <div className="pt-2 border-t border-slate-200 dark:border-slate-800/80 flex items-center justify-between text-[10px] text-slate-500 dark:text-slate-400 font-mono">
              <span>Pathologist: Dr. R. Sengupta, MD</span>
              <span className="text-teal-700 dark:text-teal-400 font-semibold">Electronic Seal: VERIFIED-OK</span>
            </div>
          </div>
          ...
          >
            ...
            <div className="bg-slate-50 dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800/80 rounded-xl p-3 grid grid-cols-2 gap-2 text-[11px]">
              ...
              <div>
                <span className="text-slate-500 dark:text-slate-400 block text-[10px]">SPECIMEN / SITE:</span>
                <span className="text-cyan-700 dark:text-cyan-300 font-medium">{patient2.specimen}</span>
              </div>
              <div>
                <span className="text-slate-500 dark:text-slate-400 block text-[10px]">COLLECTION DATE:</span>
                <span className="text-slate-700 dark:text-slate-300">{patient2.collectionDate}</span>
              </div>
            </div>
            ...
            <div className="bg-slate-50/70 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-xl p-3 space-y-1.5">
              ...
              <div className="text-sm font-bold text-slate-900 dark:text-white font-mono">{patient2.pathogen}</div>
              <div className="text-[11px] text-slate-500 dark:text-slate-400 flex items-center justify-between">
                <span>Microscopy: Abundant PMNs, Acinetobacter clusters</span>
                <span>Colony Count: &gt;10⁵ CFU/mL</span>
              </div>
            </div>
            ...
            {/* Pathologist Sign-Off */}
            <div className="pt-2 border-t border-slate-200 dark:border-slate-800/80 flex items-center justify-between text-[10px] text-slate-500 dark:text-slate-400 font-mono">
              <span>Pathologist: Dr. K. Mehta, MD</span>
              <span className="text-cyan-700 dark:text-cyan-400 font-semibold">Electronic Seal: VERIFIED-OK</span>
            </div>
          </div>
        </div>
      </div>

      {/* 4. HOSPITAL INFECTION CONTROL (HICC) WARD ACTION & COHORTING PROTOCOL */}
      <div className="rounded-2xl border border-amber-300 dark:border-amber-500/40 p-6 space-y-4 shadow-sm dark:shadow-xl bg-amber-50/30 dark:bg-slate-950/80 transition-colors">
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="text-base font-bold text-slate-900 dark:text-white">
                Hospital Infection Control Committee (HICC) Directives
              </h3>
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Action matrix for ward staff, ICU nurses, and hospital administration to prevent inter-patient outbreak.
              </p>
            </div>
          </div>
          <span className="px-3 py-1 rounded-full text-xs font-mono font-bold bg-amber-100 dark:bg-amber-500/20 text-amber-800 dark:text-amber-300 border border-amber-300 dark:border-amber-500/40 self-start sm:self-auto">
            Isolation Protocol 4B Enforced
          </span>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
          <div className="bg-white dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 rounded-xl p-4 space-y-2 shadow-sm">
            <div className="flex items-center space-x-2 text-amber-700 dark:text-amber-400 font-bold">
              <AlertTriangle className="w-4 h-4" />
              <span>Bed Cohorting &amp; Spatial Rule</span>
            </div>
            <p className="text-slate-600 dark:text-slate-300 leading-relaxed">
              <strong>Do NOT Cohort:</strong> Marcus Vance ({patient1.ward}) and Arthur Pendelton ({patient2.ward}) must remain in physically separated isolation bays. Maintain &gt;2.5m clearance or single-room isolation to avoid airborne droplet / contact transfer of carbapenemase genes.
            </p>
          </div>
          ...
          <div className="bg-white dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 rounded-xl p-4 space-y-2 shadow-sm">
            <div className="flex items-center space-x-2 text-teal-700 dark:text-teal-400 font-bold">
              ...
              <span>Dedicated Nursing Cohort</span>
            </div>
            <p className="text-slate-600 dark:text-slate-300 leading-relaxed">
              Nursing personnel assigned to Bed 04 (ICU) must NOT rotate into Step-Down Ward during the same shift. Strict 5-moment WHO hand hygiene with 4% chlorhexidine scrub before and after touching patient environment.
            </p>
          </div>
          ...
          <div className="bg-white dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 rounded-xl p-4 space-y-2 shadow-sm">
            ...
            <p className="text-slate-600 dark:text-slate-300 leading-relaxed">
              Dedicated blood pressure cuffs and pulse oximeter probes required. Daily terminal surface disinfection using 1,000 ppm active chlorine solution. 45-minute UV-C decontamination upon discharge.
            </p>
          </div>
        </div>
      </div>

      {/* 5. SCENARIO INPUT CONTROLS & DYNAMIC SIMULATION */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        {/* Scenario Input Controls Card */}
        <div
        ...
        >
          ...
          <div className="space-y-1.5">
            ...
            {showClinicalTooltip && (
              <div className="p-2.5 rounded-lg bg-cyan-50 dark:bg-cyan-950/40 border border-cyan-200 dark:border-cyan-500/30 text-xs text-cyan-900 dark:text-cyan-200 leading-relaxed">
                Gram-negative organisms like <em>E. coli</em> and <em>K. pneumoniae</em> utilize outer membrane porin deletion (OmpK35/36) and ESBL/carbapenemase enzymes to neutralize beta-lactams.
              </div>
            )}
          ...
          {/* Slider 1: Host Immunity Capacity */}
          ...
          {/* Slider 2: Prior Antibiotic Misuse */}
          <div className="space-y-2">
            <div className="flex items-center justify-between text-xs">
              <span className="font-mono text-slate-700 dark:text-slate-300 font-semibold tracking-wide text-[11px] uppercase">
          ...
          {/* Slider 3: Pathogen Virulence Index */}
          <div className="space-y-2">
            <div className="flex items-center justify-between text-xs">
              <span className="font-mono text-slate-700 dark:text-slate-300 font-semibold tracking-wide text-[11px] uppercase">
        ...
        <div className="lg:col-span-6 rounded-2xl border border-teal-200 dark:border-teal-500/30 p-6 shadow-sm dark:shadow-2xl bg-white dark:bg-gradient-to-br dark:from-slate-950 dark:via-slate-900 dark:to-[#0c1524] space-y-5 flex flex-col justify-between transition-colors">
          <div>
            ...
            {/* Dynamic Advice Note */}
            <div className="mt-4 p-3.5 rounded-xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 text-xs text-slate-700 dark:text-slate-300 leading-relaxed space-y-1">
              <div className="flex items-center space-x-1.5 text-teal-700 dark:text-teal-400 font-semibold text-[11px]">
                <ShieldCheck className="w-3.5 h-3.5" />
                <span>Simulated Pharmacodynamic Finding:</span>
              </div>
              <p>
                {simulateMissedDose
                  ? `WARNING: Skipping doses on Day 3 drops serum levels below mutant prevention concentration (MPC). The ${targetPathogen} population develops secondary chromosomal mutations, rendering ${antibioticExposure} ineffective.`
                  : `With ${immunityCapacity}% host immunity and ${virulenceIndex}% virulence, targeted continuous infusion maintains therapeutic time above MIC (T > MIC). Cross-resistance with Patient 2 must be avoided.`}
              </p>
            </div>
          </div>
      ...
      {/* 6. HOSPITAL PHARMACY ANTIMICROBIAL STEWARDSHIP (AMS) PRE-AUTHORIZATION SLIP */}
      <div className="rounded-2xl border border-slate-200 dark:border-slate-700/80 p-6 space-y-4 shadow-sm dark:shadow-xl bg-white dark:bg-slate-950 transition-colors">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-3 border-b border-slate-200 dark:border-slate-800 gap-2">
          <div className="flex items-center space-x-3">
            <div className="w-9 h-9 rounded-xl bg-teal-50 dark:bg-teal-500/20 border border-teal-200 dark:border-teal-500/30 flex items-center justify-center text-teal-600 dark:text-teal-400">
              <Stethoscope className="w-5 h-5" />
            </div>
            <div>
              <h3 className="text-base font-bold text-slate-900 dark:text-white">
                Hospital Pharmacy Antimicrobial Stewardship (AMS) Pre-Authorization Request
              </h3>
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Official hospital order slip required for dispensing restricted Reserve Category antibiotics (WHO AWaRe list).
              </p>
            </div>
          </div>
          <button
            ...
            className="inline-flex items-center space-x-1.5 px-3 py-1.5 rounded-lg bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 border border-slate-200 dark:border-slate-700 text-xs font-medium self-start sm:self-auto transition-colors cursor-pointer"
          >
            <Printer className="w-3.5 h-3.5" />
            <span>Print Pharmacy Slip</span>
          </button>
        </div>

        <div className="bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 rounded-xl p-4 text-xs font-mono space-y-3">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 border-b border-slate-200 dark:border-slate-800/80 pb-3">
            <div>
              <span className="text-slate-500 dark:text-slate-400 block text-[10px]">PATIENT 1 INDICATION:</span>
              <span className="text-slate-900 dark:text-white font-bold">{patient1.name} (UHID: {patient1.id})</span>
              <span className="text-slate-500 dark:text-slate-400 block text-[10px]">eGFR: {patient1.clinicalParams?.bloodReport.eGfr || 42} mL/min</span>
            </div>
            <div>
              <span className="text-slate-500 dark:text-slate-400 block text-[10px]">REQUESTED RESTRICTED DRUG:</span>
              <span className="text-teal-700 dark:text-teal-300 font-bold">Ceftazidime-Avibactam (Avycaz) 2.5g IV q8h</span>
              <span className="text-slate-500 dark:text-slate-400 block text-[10px]">Infusion: Extended 3-hour</span>
            </div>
            <div>
              <span className="text-slate-500 dark:text-slate-400 block text-[10px]">MICROBIOLOGICAL JUSTIFICATION:</span>
              <span className="text-amber-700 dark:text-amber-300 font-bold">KPC Carbapenemase Positive</span>
              <span className="text-slate-500 dark:text-slate-400 block text-[10px]">Meropenem MIC &gt;= 16 ug/mL</span>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-[11px] text-slate-500 dark:text-slate-400">
            <span>Approved by: Infectious Disease Lead (ID-5891)</span>
            <span>HICC Outbreak Reference: {analysis?.uniqueAccessCode || "PRP-9021-8842-8801"}</span>
            <span className="text-emerald-700 dark:text-emerald-400 font-bold">DISPENSE STATUS: PRE-AUTHORIZED</span>
          </div>
        </div>
      </div>
    </section>
