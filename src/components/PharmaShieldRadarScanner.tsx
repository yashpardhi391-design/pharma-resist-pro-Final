...
export const PharmaShieldRadarScanner: React.FC<PharmaShieldRadarScannerProps> = ({
  ...
  return (
    ...
    >
      ...
      {/* Top Left Status Badge: DATABASE READY */}
      ...
      >
        ...
        <div
          className={`absolute inset-0 rounded-full pointer-events-none transition-all ${
            scanSpeed === "boost" ? "animate-radar-sweep [animation-duration:1.5s]" : "animate-radar-sweep [animation-duration:4s]"
          }`}
          style={{
            background:
              "conic-gradient(from 0deg at 50% 50%, transparent 0deg, transparent 270deg, rgba(20,184,166,0.05) 300deg, rgba(20,184,166,0.4) 360deg)",
          }}
        >
          {/* Sweeping Leading Line */}
          <div className="absolute top-1/2 left-1/2 w-1/2 h-[1.5px] bg-gradient-to-r from-teal-300 via-cyan-400 to-white shadow-[0_0_8px_#2dd4bf] origin-left -translate-y-1/2" />
        </div>

        {/* Animated Radar Pulse Ping Wave */}
        {isPinging && (
          <div className="absolute inset-0 rounded-full border-2 border-cyan-400 animate-radar-ping pointer-events-none" />
        )}

        {/* Centerpiece: Glowing Medical Shield & Plus Sign (Exact match to screenshot) */}
        <div className="relative z-10 flex flex-col items-center justify-center">
          {/* Shield Outline Outer Glow */}
          <div className="relative w-24 h-28 flex items-center justify-center">
            {/* Custom SVG Medical Shield Contour matching the screenshot */}
            <svg
            ...
            {/* Glowing White Medical Plus (+) in the Center of Shield */}
        ...
        {blips.map((blip) => {
          ...
          // Calculate x and y percentage positions from center (50%, 50%)
          const radiusPct = (blip.distance / 100) * 44; // 44% max radius to stay inside outer circle
      ...
      {/* Status Active Badge (Bottom Right matching screenshot) */}
      ...
      {/* Card Footer: Browser lookup / Secure UI (Exact match to screenshot) */}
      ...
      {/* Interactive Quick Scan Action Button */}
      {onScanClick && (
        <div className="mt-4 relative z-10">
          <button
            onClick={onScanClick}
            className="w-full flex items-center justify-center space-x-2 py-2.5 px-4 rounded-xl bg-gradient-to-r from-teal-600 to-cyan-600 hover:from-teal-500 hover:to-cyan-500 text-white font-semibold text-xs tracking-wider uppercase transition-all shadow-md hover:shadow-teal-500/25 cursor-pointer"
          >
