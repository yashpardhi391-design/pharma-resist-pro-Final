...
export default function App() {
  const [activeTab, setActiveTab] = useState<"overview" | "scanner" | "hospital" | "analytics" | "records">("overview");
...
Empiric monotherapy with beta-lactamase inhibitor combinations or carbapenems is strictly contraindicated. Both patients share susceptibility only to novel beta-lactamase inhibitor adjuncts (Ceftazidime-Avibactam) and membrane-active polymyxins. Strict spatial segregation is advised to prevent inter-ward clonal transmission.`,
  ...
  const handleSelectScenario = (key: "icuCarbapenem" | "postOpSurgery" | "cityCentralLab") => {
    if (key === "cityCentralLab") {
      setPatient1(SAMPLE_SCENARIOS.cityCentralLab.p1);
      setPatient2(SAMPLE_SCENARIOS.cityCentralLab.p2);
      const rows = buildComparativeRows(SAMPLE_SCENARIOS.cityCentralLab.p1, SAMPLE_SCENARIOS.cityCentralLab.p2);
      setComparativeRows(rows);
    } else if (key === "icuCarbapenem") {
      setPatient1(SAMPLE_SCENARIOS.icuCarbapenem.p1);
      setPatient2(SAMPLE_SCENARIOS.icuCarbapenem.p2);
      const rows = buildComparativeRows(SAMPLE_SCENARIOS.icuCarbapenem.p1, SAMPLE_SCENARIOS.icuCarbapenem.p2);
      setComparativeRows(rows);
    } else if (key === "postOpSurgery") {
      setPatient1(SAMPLE_SCENARIOS.postOpSurgery.p1);
      setPatient2(SAMPLE_SCENARIOS.postOpSurgery.p2);
      const rows = buildComparativeRows(SAMPLE_SCENARIOS.postOpSurgery.p1, SAMPLE_SCENARIOS.postOpSurgery.p2);
      setComparativeRows(rows);
    }
  };

  return (
    <div className="min-h-screen flex flex-col bg-slate-50 dark:bg-[#0B1120] text-slate-900 dark:text-slate-100 selection:bg-teal-500/30 selection:text-teal-900 dark:selection:text-teal-200 font-sans transition-colors duration-200">
      {/* Top Navigation */}
      <Navbar
        activeTab={activeTab}
        ...
        aiOnline={aiOnline}
        onOpenCodePortal={() => handleOpenCodeLookup(analysis?.uniqueAccessCode)}
      />
      ...
      {/* Footer */}
      <footer className="no-print border-t border-slate-200 dark:border-slate-800/80 bg-white dark:bg-[#070D18] py-8 text-xs text-slate-500 dark:text-slate-400 transition-colors">
        <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex items-center space-x-3">
            <div className="w-6 h-6 rounded-lg bg-teal-50 dark:bg-teal-500/20 border border-teal-200 dark:border-teal-500/30 flex items-center justify-center text-teal-600 dark:text-teal-400">
            ...
            <span className="text-[11px] text-slate-500 dark:text-slate-400">
              Clinical Anti-Bacterial Resistance (AMR) Surveillance • EUCAST &amp; CLSI M100 Performance Standards
            </span>
          </div>

          <div className="flex items-center space-x-6 text-[11px] text-slate-500 dark:text-slate-400">
            <span className="flex items-center space-x-1">
              <HeartPulse className="w-3.5 h-3.5 text-rose-500 dark:text-rose-400" />
              <span>University Academic Project • College of Pharmacy</span>
            </span>
